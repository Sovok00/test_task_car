...
def get_currency_rate(execution_date):
    ...
    В случае ошибки делает до 3 попыток с интервалом 30 минут.
    """
    ...
    max_retries = 3
    retry_delay = 120  # 30 минут в секундах
    last_exception = None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Парсим XML правильно
            root = ET.fromstring(response.text)

            # Ищем нужную валюту по ID
            for valute in root.findall('Valute'):
                if valute.get('ID') == 'R01090B':  # USD
                    value = valute.find('Value').text
                    return float(value.replace(',', '.'))

            # Если не нашли валюту
            raise ValueError("Currency with ID R01235 not found in response")

        except Exception as e:
            last_exception = e
            logger.warning(f"Attempt {attempt} failed to get currency rate: {str(e)}")
            if attempt < max_retries:
                logger.info(f"Waiting {retry_delay/60} minutes before next attempt...")
                time.sleep(retry_delay)
            continue

    # Если все попытки исчерпаны
    logger.error(f"All {max_retries} attempts failed to get currency rate")
